$(document).ready(() => {
    ...
    if ($("#dt_basic").length) {
        const dtBasicTable = $("#dt_basic").DataTable({
            ...
            initComplete: () => {
                safeAddClass(
                    "#dt_basic_wrapper > .row.mt-2.justify-content-between",
                    "mx-2 py-2"
                );
            },
        });
    ...
    if ($("#dt_ComplexHeaders").length) {
        const dtComplexHeadersTable = $("#dt_ComplexHeaders").DataTable({
            ...
            initComplete: () => {
                safeAddClass(
                    "#dt_ComplexHeaders_wrapper > .row.mt-2.justify-content-between",
                    "mx-2 py-2"
                );
            },
        });
    ...
    if ($("#dt_AjaxData").length) {
        const dtAjaxDataTable = $("#dt_AjaxData").DataTable({
            ...
            initComplete: () => {
                safeAddClass(
                    "#dt_AjaxData_wrapper > .row.mt-2.justify-content-between",
                    "mx-2 py-2"
                );
            },
        });
    ...
    if ($("#dt_ExtraDetailed").length) {
        ...
        const dtExtraDetailedTable = $("#dt_ExtraDetailed").DataTable({
            ...
            initComplete: () => {
                safeAddClass(
                    "#dt_ExtraDetailed_wrapper > .row.mt-2.justify-content-between",
                    "mx-2 py-2"
                );
            },
        });
    ...
    if ($("#dt_ScrollVertical").length) {
        const dtScrollVerticalTable = $("#dt_ScrollVertical").DataTable({
            ...
            initComplete: () => {
                const dtSearch = $(
                    "#dt_ScrollVertical_wrapper .dt-search"
                ).detach();
                $("#dt_ScrollVertical_Search").append(dtSearch);
                $("#dt_ScrollVertical_Search .dt-search").prepend(
                    '<i class="fi fi-rr-search"></i>'
                );
                $("#dt_ScrollVertical_Search .dt-search label").remove();
                $(
                    "#dt_ScrollVertical_wrapper > .row.mt-2.justify-content-between"
                )
                    .first()
                    .remove();
            },
