    ...
    series: [
        {
            name: "Gross Salary",
            data: [
                55000, 60000, 68000, 52000, 69000, 73000, 71000, 74000, 70000,
                75000, 72000, 77000,
            ],
        },
        {
            name: "Net Salary",
            data: [
                25000, 27000, 16000, 15500, 16500, 17200, 15800, 17000, 16200,
                16800, 16000, 17500,
            ],
        },
        {
            ...
            type: "line",
            data: [
                25000, 50000, 40000, 20000, 52500, 45000, 55200, 47000, 25800,
                58200, 36000, 59500,
            ],
        },
    ],
...
if (
    typeof leavesUnplannedScore !== undefined &&
    leavesUnplannedScore !== null
) {
    const chartInit = new ApexCharts(
        leavesUnplannedScore,
        leavesUnplannedScoreConfig
    );
    chartInit.render();
}
...
if (typeof leavesPendingScore !== undefined && leavesPendingScore !== null) {
    const chartInit = new ApexCharts(
        leavesPendingScore,
        leavesPendingScoreConfig
    );
    chartInit.render();
}

const bsBodyBG = getComputedStyle(document.documentElement)
    .getPropertyValue("--bs-body-bg")
    .trim();
const bsPrimary = getComputedStyle(document.documentElement)
    .getPropertyValue("--bs-primary")
    .trim();
const bsSecondary = getComputedStyle(document.documentElement)
    .getPropertyValue("--bs-secondary")
    .trim();
const bsBodyColor = getComputedStyle(document.documentElement)
    .getPropertyValue("--bs-body-color")
    .trim();
const bsBorderColor = getComputedStyle(document.documentElement)
    .getPropertyValue("--bs-border-color")
    .trim();
const bsHeadingColor = getComputedStyle(document.documentElement)
    .getPropertyValue("--bs-heading-color")
    .trim();
const bsFontFamily = getComputedStyle(document.documentElement)
...
if ($("#dt_EmployeeLeave").length) {
    const dt_EmployeeLeave = $("#dt_EmployeeLeave").DataTable({
        ...
        initComplete: function () {
            ...
            $("#dt_EmployeeLeave_Search .dt-search").prepend(
                '<i class="fi fi-rr-search"></i>'
            );
            $("#dt_EmployeeLeave_Search .dt-search label").remove();
            $("#dt_EmployeeLeave_wrapper > .row.mt-2.justify-content-between")
                .first()
                .remove();
        },
    });
...
if ($("#dt_PageEmployeeLeave").length) {
    const dt_PageEmployeeLeave = $("#dt_PageEmployeeLeave").DataTable({
        ...
        initComplete: function () {
            var dtSearch = $(
                "#dt_PageEmployeeLeave_wrapper .dt-search"
            ).detach();
            $("#dt_PageEmployeeLeave_Search").append(dtSearch);
            $("#dt_PageEmployeeLeave_Search .dt-search").prepend(
                '<i class="fi fi-rr-search"></i>'
            );
            $("#dt_PageEmployeeLeave_Search .dt-search label").remove();
            $(
                "#dt_PageEmployeeLeave_wrapper > .row.mt-2.justify-content-between"
            )
                .first()
                .remove();
        },
...
if ($("#dt_EmployeeAttendance").length) {
    const dt_EmployeeAttendance = $("#dt_EmployeeAttendance").DataTable({
        ...
        initComplete: function () {
            var dtSearch = $(
                "#dt_EmployeeAttendance_wrapper .dt-search"
            ).detach();
            $("#dt_EmployeeAttendance_Search").append(dtSearch);
            $("#dt_EmployeeAttendance_Search .dt-search").prepend(
                '<i class="fi fi-rr-search"></i>'
            );
            $("#dt_EmployeeAttendance_Search .dt-search label").remove();
            $(
                "#dt_EmployeeAttendance_wrapper > .row.mt-2.justify-content-between"
            )
                .first()
                .remove();
        },
...
if ($("#dt_PayrollList").length) {
    const dt_PayrollList = $("#dt_PayrollList").DataTable({
        ...
        initComplete: function () {
            ...
            $("#dt_PayrollList_Search .dt-search").prepend(
                '<i class="fi fi-rr-search"></i>'
            );
            $("#dt_PayrollList_Search .dt-search label").remove();
            $("#dt_PayrollList_wrapper > .row.mt-2.justify-content-between")
                .first()
                .remove();
        },
        columnDefs: [
