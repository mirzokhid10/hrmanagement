    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'email' => [
                ...
                'email',
                'max:255',
                // Email must be unique for users (login accounts) within the tenant
                Rule::unique('users', 'email')->where(function ($query) use ($tenantId) {
                    return $query->where('company_id', $tenantId);
                }),
                // Email must also be unique for employee profiles within the tenant
            ...
            'department_id' => [
                ...
                // Ensure department belongs to the current tenant
                Rule::exists('departments', 'id')->where(function ($query) use ($tenantId) {
                    $query->where('company_id', $tenantId);
                }),
            ],
            'job_title' => ['required', 'string', 'max:255'],
            'hire_date' => ['required', 'date_format:Y-m-d'], // Changed to date_format
            'status' => ['required', 'string', Rule::in(['Active', 'Inactive', 'Probation', 'Terminated'])],
            'date_of_birth' => ['nullable', 'date_format:Y-m-d'], // Changed to date_format
            'address' => ['nullable', 'string', 'max:500'],
            'image' => ['nullable', 'image', 'mimes:jpeg,png,jpg,gif,svg', 'max:2048'], // Using 'image'
