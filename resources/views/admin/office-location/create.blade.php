        ...
        <div class="card-body">
            <form action="{{ route('admin.office-location.store') }}" method="POST">
                ...
                <div class="row">
                    ...
                    @else
                        {{-- Hidden company_id for regular users (HR managers) --}}
                        {{-- This input is technically not needed if tenant()->id is always used in controller --}}
                        {{-- but can be a fallback or for clarity. The controller's logic will override this. --}}
                        <input type="hidden" name="company_id" value="{{ auth()->user()->company_id ?? tenant()->id }}">
                    @endif
                ...
                <div class="row">
                    <div class="mb-3 col-12 col-md-6">
                        ...
                        <input type="number" name="radius_meters" id="radius_meters"
                            ...
                            value="{{ old('radius_meters', 50) }}" min="10" max="1000">
                        @error('radius_meters')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="text-end">
                    ...
                    <button type="submit" class="btn btn-primary">Add Office Location</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function init() {
            ...
            if (oldLatitude && oldLongitude) {
                const initialCoords = [parseFloat(oldLatitude), parseFloat(oldLongitude)];
                placemark = new ymaps.Placemark(initialCoords, {}, {
                    preset: 'islands#blueDotIcon'
                });
                map.geoObjects.add(placemark);
                map.setCenter(initialCoords, defaultZoom);
            }
